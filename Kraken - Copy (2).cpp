...
DOGE/BTC,50,0.0001,0,0,0.00000001,0.01,0,1,1,0.0001,XDG/XBT
...
- History from cryptocompare use aggregate and dateTo
- fix invalid key
...
int BrokerHistory3(std::string sAsset, DATE tStart, DATE tEnd, int nTickMinutes, int nTicks, T6* ticks)
{
	...
	//https://min-api.cryptocompare.com/data/histoday?fsym=ETH&tsym=BTC&limit=2000&aggregate=1&e=CCCAGG
	...
	try
	{
		...
		for (picojson::value::array::const_iterator iC = objData.end(); iC != objData.begin(); --iC)
		{
			...
			//Set Volume
			int volumeFrom = atof(iC->get("volumefrom").to_str().c_str());
			int volumeTo = atof(iC->get("volumeto").to_str().c_str());
			tick->fVol = (volumeFrom + volumeTo)/2;
...
DLLFUNC int BrokerBuy(char* Asset, int nAmount, double dStopDist, double *pPrice)
{
	...
	std::string sAsset = GetAsset(Asset);
	Log("BrokerBuy Params: " + sAsset + " Amount:" + itoa(nAmount) + " topDist:" + ftoa(dStopDist),0,false);
