...
DemoTrading is to allow the Buy/Sell to be only validated by the broker but would not place an order
EnableLog is to log to KrakenPlugin.log 
...
MinOrderSize={"error":[],"result":{"REP":0.3,"XBT":0.002,"DASH":0.03,"XDG":3000,"EOS":2,"ETH":0.02,"ETC":0.3,"GNO":0.03,"ICN":2,"LTC":0.1,"MLN":0.1,"XMR":0.1,"XRP":30,"XLM":300,"ZEC":0.03,"BCH":0.002}}

Name,Price,Spread,RollLong,RollShort,PIP,PIPCost,MarginCost,Leverage,LotAmount,Commission,Symbol
BCH/BTC,50,0.0001,0,0,0.00000001,0.01,0,1,1,0.0001,BCH/XBT
DASH/BTC,50,0.0001,0,0,0.00000001,0.01,0,1,1,0.0001,DASH/XBT
EOS/BTC,50,0.0001,0,0,0.00000001,0.01,0,1,1,0.0001,EOS/XBT
ETC/BTC,50,0.0001,0,0,0.00000001,0.01,0,1,1,0.0001,ETC/XBT
GNO/BTC,50,0.0001,0,0,0.00000001,0.01,0,1,1,0.0001,GNO/XBT
ICN/BTC,50,0.0001,0,0,0.00000001,0.01,0,1,1,0.0001,ICN/XBT
LTC/BTC,50,0.0001,0,0,0.00000001,0.01,0,1,1,0.0001,LTC/XBT
MLN/BTC,50,0.0001,0,0,0.00000001,0.01,0,1,1,0.0001,MLN/XBT
REP/BTC,50,0.0001,0,0,0.00000001,0.01,0,1,1,0.0001,REP/XBT
XLM/BTC,50,0.0001,0,0,0.00000001,0.01,0,1,1,0.0001,XLM/XBT
XMR/BTC,50,0.0001,0,0,0.00000001,0.01,0,1,1,0.0001,XMR/XBT
XRP/BTC,50,0.0001,0,0,0.00000001,0.01,0,1,1,0.0001,XRP/XBT
ZEC/BTC,50,0.0001,0,0,0.00000001,0.01,0,1,1,0.0001,ZEC/XBT
ETH/BTC,50,0.0001,0,0,0.00000001,0.01,0,1,1,0.0001,ETH/XBT
DOGR/BTC,50,0.0001,0,0,0.00000001,0.01,0,1,1,0.0001,XDG/XBT
...
Also in the Zorro log and Html log only 0
...
std::string GetAPISignature(std::string ApiPath, std::string PostData, std::string nonce)
{
	...
	const int len = ApiPath.length() + SHA256::DIGEST_SIZE + 1;
	...
	HMAC512(encp, std::string((char *)path_param), hmac_512);
...
int CallAPI(std::string ApiPath, std::string& json, std::string symbol, std::string data, bool UseCache)
{
	...
	std::string URL = _KrakenUrl + ApiPath;
...
int Parse(std::string ApiPath, picojson::value::object& objJson, std::string& json, std::string symbol, std::string data)
{
	try
	{
		...
		if (ApiPath.find("/AssetPairs") == std::string::npos && ApiPath.find("/OHLC") == std::string::npos)
			Log(json, 0, false);
...
int CallAPIAndParse(std::string ApiPath, picojson::value::object& objJson, std::string symbol, std::string data, bool UseCache)
{
	...
	try
	{
		...
		std::string json;
		if (!CallAPI(ApiPath, json, symbol, data, UseCache))
		{
